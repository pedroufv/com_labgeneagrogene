 ...
 * @since  1.6
 */
class LabgeneagrogeneModelExamslist extends JModelAdmin
{
    /**
     ...
     * @since    1.6
     */
    public function getTable($type = 'Examslist', $prefix = 'LabgeneagrogeneTable', $config = array())
    {
    ...
    public function getForm($data = array(), $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm(
            'com_labgeneagrogene.examslist', 'examslist',
            array('control' => 'jform',
                'load_data' => $loadData
            )
        );
    ...
    public function getCategoriesInRequest($requestid)
    {
        ...
        $query->from('`#__labgeneagrogene_examslist` AS examslist');
        $query->innerJoin('`#__labgeneagrogene_exams` AS `exams` ON `exams`.id = examslist.`examsid`');
